...
export default function ContactForm() {
    const [formContent, setFormContent] = useState<FormContent>({ name: "", email: "", subject: "", text: "" })
    ...
    useEffect(() => {
        if (isSuccess) setFormContent({ name: "", email: "", subject: "", text: "" })
    }, [isSuccess])

    return (
        <form onSubmit={onFormSubmission} className='contact-form'>
            <label className='contact-form__label' htmlFor="name">Imię:</label>
            <input 
                id="name"
                name="name"
                type="text"
            ...
            <input 
                ...
                required
            />
            <label className='contact-form__label' htmlFor="text">Opis:</label>
            <textarea 
                id="text"
                name="text"
                maxLength={500}
            ...
            <button 
                ...
                disabled={isLoading}
            >
                {!isLoading ? 'Wyślij' : <p className="contact-form__spinner"></p>}
            </button>
            {isSuccess && !isError && <div className='contact-form__success'><p>✓</p><div>Wysłano poprawnie</div></div>}
            {!isSuccess && isError && <div className='contact-form__error'><p>X</p><div>Błąd wysyłania forumlarza</div></div>}
        </form>
  )
}